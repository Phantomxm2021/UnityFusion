...
namespace UnityFusion.Editor.ReversePInvokeWrap
{
        ...
        private void CollectReversePInvokeMethods()
        {
            foreach (var mod in _rootModules)
            {
                ...
                for (uint rid = 1, n = mod.Metadata.TablesStream.MethodTable.Rows; rid <= n; rid++)
                {
                    ...
                    {
                        Method = method,
                        GenerationAttribute = method.CustomAttributes.FirstOrDefault(ca => ca.AttributeType.FullName == "UnityFusion.ReversePInvokeWrapperGenerationAttribute"),
                    });
                }
            }
        }
