...
using UnityFusion.Editor.Template;
using System;
...
namespace UnityFusion.Editor.Il2CppDef
{
    public class Il2CppDefGenerator
    {
        ...
        private void GenerateIl2CppConfig()
        {
            ...
            lines.Add($"#define UNITYFUSION_UNITY_VERSION {majorVer}{minorVer1.ToString("D2")}{minorVer2.ToString("D2")}");
            lines.Add($"#define UNITYFUSION_UNITY_{majorVer} 1");
            for (int ver = 2019; ver <= 2024; ver++)
            {
                if (majorVer >= ver)
                {
                    lines.Add($"#define UNITYFUSION_UNITY_{ver}_OR_NEW 1");
                }
            }
            ...
            Debug.Log($"[UnityFusion.Editor.Il2CppDef.Generator] output:{_options.OutputFile}");
        }
        ...
        private void GeneratePlaceHolderAssemblies()
        {
            ...
            Debug.Log($"[UnityFusion.Editor.Il2CppDef.Generator] output:{_options.OutputFile2}");
        }
    }
}
